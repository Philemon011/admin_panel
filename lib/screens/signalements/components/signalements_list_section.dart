...
import 'package:admin/screens/signalements/components/historique_screen.dart';
import '../../../core/data/data_provider.dart';
import 'view_signalements_form.dart';
                  ...
                  rows: List.generate(
                    ...
                    (index) => SignalementDataRow(
                      ...
                      showSignalementInfo(
                       ...
                       Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => HistoriqueScreen(signalementId: dataProvider.signalements[index].id??0)),
    );
                    }),
                  ),
                );
...
DataRow SignalementDataRow(Signalement signalementInfo, int index,
    {Function? viewHistory, Function? viewMore}) {
  return DataRow(
    cells: [
      DataCell(
        Row(
      ...
      DataCell(Text(signalementInfo.libelle ?? '')),
      DataCell(Text(signalementInfo.createdAt ?? '')),
      DataCell(Container(
        padding: EdgeInsets.symmetric(vertical: 8.0, horizontal: 12.0),
        decoration: BoxDecoration(
          color: signalementInfo.cloturer_verification == 'oui'
              ? Colors.green.withOpacity(0.1) // Vert clair
              : Colors.red.withOpacity(0.1), // Rouge clair
          borderRadius: BorderRadius.circular(8.0),
        ),
        child: Text(
          signalementInfo.cloturer_verification ?? '',
          style: TextStyle(
            color: signalementInfo.cloturer_verification == 'oui'
                ? Colors.green // Texte vert foncé pour "oui"
                : Colors.red, // Texte rouge foncé pour "non"
      ...
      DataCell(IconButton(
          onPressed: () {
            if (viewHistory != null) viewHistory();
          },
          icon: Icon(
            Icons.history,
            color: Colors.white,
